...
| enable thinking    | —      | —        | Enables "thinking mode" if supported by the model. Depends on the parameter protocol of the model service side. <br>• DashScope: `enable_thinking=True` together with `thinking_budget` (a positive integer ≤ the model's max thinking length; omitting it causes a 400 error) <br>• OpenAI-compatible API of DashScope: `'extra_body': {'enable_thinking': True, 'thinking_budget': 12000}` <br>• OpenAI-compatible API of vLLM: `'extra_body': {'chat_template_kwargs': {'enable_thinking': True}}` |
